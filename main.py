...
def echo_message(message: Message):
    ...
    if storage[iD]['rooms'][1] == 0 and room == 1:
        bot.send_message(
    ...
    if storage['graph'][room]['spec'] == 0 and storage['graph'][room]['lock'][text] == 0:
        bot.send_message(
            message.from_user.id,
            'вы прошли через дверь ' + text,
        )
        storage[iD]["room"] = storage['graph'][room][text]
    elif room == 4:
        if storage['graph'][room]['lock'][text] - storage[iD]['key'] == 0:
            bot.send_message(
                    message.from_user.id,
        ...
        if storage['graph'][room]['lock'][text] - storage[iD]['code'] == 0:
            bot.send_message(
                message.from_user.id,
                'вы прошли через дверь ' + text,
            )
